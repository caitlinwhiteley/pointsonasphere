# python code to compute points in a ball using a genetic algorithm
...
random.seed()
x1 = (2*(n**(1/3))*np.random.random((n,3))-1*(n**(1/3)))
x2 = (2*(n**(1/3))*np.random.random((n,3))-1*(n**(1/3)))
x3 = (2*(n**(1/3))*np.random.random((n,3))-1*(n**(1/3)))
x4 = (2*(n**(1/3))*np.random.random((n,3))-1*(n**(1/3)))

# calculate the initial energy
energy1=0.0
for i in range(0,n):
    for j in range(i+1,n):
        distance = abs(sum(x1[i]-x1[j]))
        energy1=energy1+1.0/distance
for i in range(0,n):
    r_i = np.dot(x1[i],x1[i])
    energy1 = energy1 + 0.5*r_i

energy2 =  calcenergy(x2)
energy3 =  calcenergy(x3)
energy4 =  calcenergy(x4)
...
lowestenergy, lowestconfig = bubblesort(loweste, lowestx)
